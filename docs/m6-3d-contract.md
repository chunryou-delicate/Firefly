...
| `body` | uint32 | n | bodyId. **v1.3에서 추가.** 유지 뉴런의 실제 최대값은 1,471,062,202(어노테이션 전체로는 1,571,825,087)로 uint32 한계 안이다. 범위를 넘는 값은 자르지 말고 예외 |
...
- **v1.3** (2026-09-19): 점구름에 `body`(uint32, bodyId) 배열 추가. 배열 순서는 pos·set·src **뒤**이고 기존 셋의 오프셋은 바뀌지 않는다. 뉴런을 클릭해 식별하려면 인덱스만으로는 부족하다. 소비자는 이 배열이 없는 옛 파일도 견뎌야 한다(있으면 쓰고 없으면 인덱스만 표시).
