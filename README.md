...
### 적응 (2026-09-16 ~ 09-19, 여섯 라운드)

적응을 전류로 넣은 첫 시도는 기각했고, 전도도로 다시 만들어 여섯 라운드를 돌렸다. 각 라운드는 실행 전에 판정 기준을 문서에 박고 시작했다([m2c](docs/m2c-brief.md) · [m2d](docs/m2d-brief.md) · [m2e](docs/m2e-brief.md) · [m2f](docs/m2f-brief.md) · [m2g](docs/m2g-brief.md) · [m2h](docs/m2h-brief.md) · [m2i](docs/m2i-brief.md)).

**합동 진술.** `g` = 5.365e-4, σ 23~29에서 쓸 수 있는 저발화 배경은 **적응이 있어야만 존재한다**. 그 배경 안에서 클릭 트레인은 **홉 2(WED)에 phase-lock에서 도달하고 rate에서는 도달하지 않는다.** **pC1에는 끝내 도달하지 않는다.**

| 라운드 | 얻은 것 |
|---|---|
| 적응 전류 | 기각 — 막전위 −834 mV, 목표 상태의 96~100 %가 발화 중 |
| 적응 전도도 | 막전위가 구조적으로 [−75, −50] 안에 갇힘. 552런 위반 0 |
| 격자 해소 | 선택 규칙 자체가 결함 — 매끄러운 여유 + 딱딱한 문턱은 가장 안 강건한 점을 고른다 |
| 규칙 교체 | 표본 외 시드 5개로 검증한 강건한 동작점 확보 |
| 청각 경로 적용 | 홉 2 미검출 — 그러나 검정 바닥이 신호보다 높았다고 스스로 단서를 남김 |
| 검정력 보강 | 그 단서가 맞았다. 홉 2 도달 확인, 홉 3은 여전히 0 |
| 교란 확인 | 단일 변수 대조가 존재하지 않음. 앞 라운드의 "약화" 주장 철회 |

**홉 2에서 홉 3으로 가는 벽은 모든 구성에서 살아남았다.** pC1이 0이 아니었던 유일한 구성조차 도달이 아니라 폭주였다. 파라미터가 아니라 배선만으로 만든 모델에 대한 진술로 다룰 시점이다. 결정 이력 전체는 [parameter-decisions.md](data-provenance/parameter-decisions.md)에.

### 적응 전류 첫 시도 (2026-09-16, 기록만 하고 미채택)
