...
def test_layout_offsets_and_roundtrip(fake, tmp_path):
    ...
    assert (tmp_path / "neurons-3d.bin").stat().st_size == 14 * n
...
def test_too_many_sets_is_refused(fake, tmp_path):
    ...
    pos, src, _ = E3.neuron_positions_3d(graph, roi)
    with pytest.raises(ValueError):
        E3.build_doc(pos, np.zeros(graph.n, np.uint8), src, [f"s{i}" for i in range(257)])
    with pytest.raises(ValueError):
        E3.build_doc(pos[:-1], np.zeros(graph.n, np.uint8), src, ["a"])
...
def test_all_positions_missing_is_refused(tmp_path):
    ...
    with pytest.raises(ValueError):
        E3.build_doc(pos, np.zeros(n, np.uint8), src, ["rest"])
