...
def run_bench(steps: int = 10_000, warmup: int = 100, seed: int = 0) -> dict:
    if not torch.cuda.is_available():
        raise SystemExit("bench requires CUDA (the target is the GPU kernel)")
    graph = Graph.load()
    n = graph.n
    print(f"graph: {graph!r}")
    params = EngineParams(dt=0.1, g=0.0, t_ref=0.0)
    ...
    for p in ACTIVITY_LEVELS:
        ...
        for name, eng in engines.items():
            for record in (False, True):
                ...
                rows.append(dict(backend=name, activity=p, forced_neurons=k, record=record,
...
def markdown_table(result: dict) -> str:
    lines = ["| backend | activity | forced neurons | recorder | steps | us/step | spikes/step | transient peak MiB | <100us |",
             "|---|---|---|---|---|---|---|---|---|"]
    for r in result["rows"]:
        lines.append(f"| {r['backend']} | {r['activity']:.1%} | {r['forced_neurons']:,} | "
...
def main() -> None:
    ...
    a = ap.parse_args()
    res = run_bench(a.steps, a.warmup)
